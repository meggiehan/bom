...
(function(){
    var effectC = {
        ...
        Bounce: {
            easeIn: function (t, b, c, d) {
                return c - effectC.Bounce.easeOut(d - t, 0, c, d) + b;
            },
            ...
            easeInOut: function (t, b, c, d) {
                if (t < d / 2) {
                    return effectC.Bounce.easeIn(t * 2, 0, c, d) * .5 + b;
                }
                return effectC.Bounce.easeOut(t * 2 - d, 0, c, d) * .5 + c * .5 + b;
            }
        },
        //二次方的缓动
        Quad: {
    ...
    function move(curEle,target,duration,effect,callback){
        var tmpEffect=effectC.Linear;
        var ary = ["Linear", "Elastic-easeOut", "Back-easeOut", "Bounce-easeOut", "Expo-easeIn"];
        if(typeof effect==='number'){
            var str=ary[effect%ary.length];
            ary=str.split('-');
            tmpEffect=ary.length>=2?effectC[ary[0]][ary[1]]:effectC[ary[0]];
        }else if(typeof effect==='object'){
            tmpEffect=effect.length>=2?effectC[effect[0]][effect[1]]:effectC[effect[0]];
