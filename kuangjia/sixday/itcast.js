(function(window,undefined){
	...
	itcast.fn = itcast.prototype = {
		...
		init:function(html){
			//为所有的itcast对象添加events成员，用来存储事件处理函数
			this.events = {};
			...
			//itcast( function(){   ...   } )
			/*
				itcast(function(){  console.log("快中午了") });
				itcast(function(){  console.log("该吃饭了") });
			...
			*/
			if( typeof html === 'function'){
				...
				if( typeof oldFn === 'function'){
					...
					//然后再调用新的函数处理html，以此完成事件追加
					window.onload = function(){
						oldFn();
						html();
					}
				}
				//如果oldFn不是一个函数，代表onload没有绑定任何的事件处理
				else
				{
					window.onload = html;
				}
...
//事件处理模块
//var iObj = itcast("div");  iObj.click(function(){ alert("你点了一个div");  })
//iObj.click(function(){ alert("点什么点");  })
itcast.fn.extend({
	click:function(func){
		var iObj = this;
		//判断一下events对象中的click数组是否不存在，如果不存在，将其进行初始化
		if(  !iObj.events['click'] )
		{	
			iObj.events['click'] = [];
			
			//下面的this指的是调用click方法的itcast对象iObj
			iObj.each(function(){
				//下面的这个this是遍历的每一个div元素
				this.onclick = function(){
					//循环遍历click数组中的每一个函数，并调用
		...
		//将事件处理的函数添加到events对象中的click数组中
		iObj.events['click'].push(func);

		//返回当前的itcast对象 以支持链式编程
		return iObj;


		/*
		//下面一行的this是click函数中的this，所以this指的是调用click函数的itcast对象
		this.each(function(){
			//这个this是each函数中的this。这里的this指的是itcast对象中的dom元素
			this.onclick = func;
		});
	...
	on:function(type,func){
		...
		{	
			...
			iObj.each(function(){
				...
				//如果dom对象支持addEventListener方法，就用该方法绑定函数
	...
	toggle:function(){
		...
		this.on("click",function(e){
			//使用this对象(发生事件的dom对象) 调用args中的函数，并传递事件参数e
...
itcast.fn.extend({
	css:function(option){
		...
		{	
			...
			{
				return this[0].style[ args[0] ] || itcast.getStyle( this[0], args[0] ) ;
			}
			//否则就是B形式的css方法调用
			else if( itcast.isObject(args[0]) )
			{
				//循环遍历this对象中的每一个dom元素
				for(var i=0;i<this.length;i++){
					//循环遍历option对象中的键
					for(var k in option){
	...
	toggleClass:function(name){
		//如果当前itcast对象中的元素中包含name类名
		if(this.hasClass(name))
		{	//将元素中的name类名删除
			this.removeClass(name);
		}
		else
		{	//否则就是没有这个类名，那么就添加这个类名
			this.addClass(name);
		}
		//返回当前itcast对象以备链式编程使用
		return this;
	}
})
...
//通过循环遍历绑定其他事件处理的代码
itcast.each(  "mouseout mouseenter mousemove keydown".split(" "),function(){
		//itcast.fn[this]
		// console.log(this);
		//这里的this指的是each遍历的事件名称
		var eventName = this;

		itcast.fn[eventName] = function(fn){
			//这个this是在原型的方法中的this，想要调用原型中的方法，我们必须使用实例对象调用
			//所以在这个方法中的this就是itcast对象
			var iObj = this;
			//为iObj对象中的所有元素添加eventName事件类型的事件处理函数fn。
